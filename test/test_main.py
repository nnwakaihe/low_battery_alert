...
def test_check_battery_and_notify_low_battery():
    ...
    with patch("your_script_name.sensors_battery") as mock_battery, \
            patch("your_script_name.show_alert") as mock_show_alert:
...
def test_play_audio():
    ...
    with patch("your_script_name.winsound.PlaySound") as mock_play_sound:
        audio_playing = [True]
...
def test_show_alert():
    ...
    with patch("your_script_name.Tk") as mock_tk, \
            patch("your_script_name.Button"), \
            patch("your_script_name.Label"), \
            patch("your_script_name.threading.Thread"):
        mock_root = MagicMock()
